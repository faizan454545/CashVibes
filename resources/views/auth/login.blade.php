...
<main class="auth-container px-md">
    <div class="w-full max-w-[420px] z-10 flex flex-col items-center">
        ...
        @if(session('success'))
        <div class="w-full mb-lg p-md border border-secondary bg-secondary-container/20 rounded">
        ...
        <div class="w-full bg-surface-container-lowest border border-outline-variant p-lg shadow-2xl">
            <div class="mb-lg">
                <h2 class="font-headline-md text-headline-md text-on-surface">Secure Sign In</h2>
                <p class="font-body-sm text-body-sm text-on-surface-variant mt-xs">Access your digital vault and market insights.</p>
            </div>
            ...
            <div class="my-lg flex items-center gap-md">
                ...
                <span class="font-label-mono text-[10px] text-on-surface-variant uppercase tracking-widest">or sign in with email</span>
                <div class="flex-1 h-[1px] bg-outline-variant"></div>
            </div>

            <form id="email-login-form" method="POST" action="{{ route('email.login') }}">
                @csrf
                <div class="mb-md">
                ...
                <button type="submit" id="email-login-btn"
            ...
            <div class="mt-md text-center">
                <p class="font-label-mono text-[11px] text-on-surface-variant">
                    Don't have an account? <a href="{{ route('register') }}" class="text-secondary hover:text-white transition-colors">Sign Up</a>
                </p>
            </div>
        </div>
...
<script>
    ...
    // ========================================
    // Firebase Auth (with domain error handling)
    // ========================================
    ...
    try {
        ...
        showToast('Google Sign-In unavailable. Use email login below.');
    }

    // Google Sign-In button
    document.getElementById('google-login-btn').addEventListener('click', async function() {
        if (!firebaseReady || !auth) {
            showToast('Google Sign-In is not configured for this domain. Use email login.');
            return;
        }
        ...
        try {
            ...
            if (data.success) {
                ...
                showToast(data.message || 'Authentication failed. Please try again.');
            }
            ...
            if (error.code === 'auth/unauthorized-domain') {
                showToast('Google Sign-In is not authorized for this domain (' + window.location.hostname + '). Use email login below, or add this domain to Firebase Console > Authentication > Authorized Domains.');
                ...
                showToast('Google Sign-In failed: ' + (error.message || 'Unknown error') + '. Use email login below.');
            }
        } finally {
            btn.disabled = false;
